			...
			if (block) {
				...
				for (chain = 0; chain < ma->numChains; chain++) {
					if (ma->residues[chain].res[low].exists == 0 ||
					    ...
					    ma->chains[chain]->res[ma->residues[chain].res[low].index + 1].exists == 0)
						maxDir = 0;
				}
...
static double AssembleSet(int *out, MultipleAlignment *ma, int *starts, int *ends, int chain, int residue, double rmsd, int length, int full)
{
	...
	for (residue2 = residue; residue2 < length + residue; residue2++) {
		if (!ma->chains[chain]->res[residue2].exists)
			return 10e10;
	}
	 ...
	 */
	if (!numChainsWithPositions || sum > 2 * numChainsWithPositions * rmsd)
		return 10e100;

	if (full == 2 && numChainsWithPositions != ma->numChains - 1)
		return 10e100;
...
double lengthSubVect(const Vector *p1, const Vector *p2)
{
        Vector v1 = {p1->x, p1->y, p1->z};
        Vector v2 = {p2->x, p2->y, p2->z};
        double l = (v1.x - v2.x) * (v1.x - v2.x) + (v1.y - v2.y) * (v1.y - v2.y) + (v1.z - v2.z) * (v1.z - v2.z);
        return sqrt(l);
}
...
void FinalExtend(MultipleAlignment *ma, double rmsd)
{
	...
	for (int length = 4; length >= 1; length--) {
		for (int block = 0; block <= ma->numBlocks; block++) {
			double best = length * ma->numChains * 100000;
			ResiduePosition *fuck = ma->residues[0].res;
			for (int chain = 0; chain < ma->numChains; chain++) {
				if (block)
					starts[chain] = ma->residues[chain].res[ma->blocks[block - 1].last].index + 1;
				else
					starts[chain] = 0;
			...
			for (int chain = 0; chain < ma->numChains; chain++) {
				for (int pos = starts[chain]; pos <= ends[chain] - length + 1; pos++) {
					double sum = 0;
					current[chain] = pos;
					#pragma omp parallel for reduction(+: sum)
					for (int chain2 = 0; chain2 < ma->numChains; chain2++) {
						double best2;
						if (chain2 == chain)
							continue;
						best2 = 100000 * length * ma->numChains;
						for (int pos2 = starts[chain2]; pos2 <= ends[chain2] - length + 1; pos2++) {
							double d = 0;
							for (int i = 0; i < length; i++) {
								...
								d += lengthSubVect(&p1->coords, &p2->coords);
							}
							if (d < best2) {
								current[chain2] = pos2;
								best2 = d;
					...
					if (sum / (ma->numChains - 1) < rmsd * 2) {
						...
						sum = 0;
						for (int chain2 = 0; chain2 < ma->numChains; chain2++) {
							for (int chain3 = chain2 + 1; chain3 < ma->numChains; chain3++) {
								double d = 0;
								for (int i = 0; i < length; i++) {
			...
			if (best > rmsd)
				...
				AlignedBlock *g;
				for (int i = block; i < ma->numBlocks; i++) {
...
double CalcSetRMSD(MultipleAlignment *ma, ResidueSet *sets1, ResidueSet *sets2, int *c, struct index_t *a, struct index_t *b) {
	int n_a = 0;
	int n_b = 0;
	for (int i = 0; i < ma->numChains; i++) {
		int inda = sets1->indices[i];
		a[n_a].index = inda;
		a[n_a].order = i;
		n_a += (inda >= 0);
		
		int indb = sets2->indices[i];
		b[n_b].index = indb;
		b[n_b].order = i;
		n_b += (indb >= 0);
	}
	...
	int count = n_a * n_b;
	for (int i = 0; i < n_a; i++) {
		Vector v1 = ma->chains[a[i].order]->res[a[i].index].coords;
		for (int j = 0; j < n_b; j++) {
			Vector v2 = ma->chains[b[j].order]->res[b[j].index].coords;
			double l = (v1.x - v2.x) * (v1.x - v2.x) + (v1.y - v2.y) * (v1.y - v2.y) + (v1.z - v2.z) * (v1.z - v2.z);
			sum += sqrt(l);
		}
	}
...
struct setRMSD_t *precalc(MultipleAlignment *ma, ResidueSet *s1, int n1, ResidueSet *s2, int n2) {
	if (n1 == 0 || n2 == 0) {
		return NULL;
	}
	struct setRMSD_t *ret = malloc(n1 * n2 * sizeof(struct setRMSD_t));
	
	#pragma omp parallel
	{
		struct index_t *buf_a = malloc(ma->numChains * sizeof(struct index_t));
		struct index_t *buf_b = malloc(ma->numChains * sizeof(struct index_t));
		#pragma omp for collapse(2) schedule(dynamic)
		for (int i = 0; i < n1; i++) {
			for(int j = 0; j < n2; j++) {
				int count;
				double rmsd = CalcSetRMSD(ma, s1 + i, s2 + j, &count, buf_a, buf_b);
				int ind = i * n2 + j;
				ret[ind].count = count;
				ret[ind].rmsd = rmsd;
			}
...
void AlignSets(MultipleAlignment *ma, int *starts, int *ends, ResidueSet **sets, int *numSets, ResidueSet *sets1, int numSets1, ResidueSet *sets2, int numSets2, double rmsd) {
	...
	if (numSets1 && numSets2) {
		struct setRMSD_t *setrmsd = precalc(ma, sets1, numSets1, sets2, numSets2);
		SetAlignmentInfo *s1Info, *lastS1Info;
		for (int s1 = 0; s1<numSets1; s1++) {
			s1Info = info + s1*numSets2;
			lastS1Info = s1Info - numSets2;
			for (int s2 = 0; s2 < numSets2; s2++) {
				struct setRMSD_t tmp = setrmsd[s1 * numSets2 + s2];
				int count = tmp.count;
				double setRmsd = tmp.rmsd;
...
void Merge(MultipleAlignment *ma, int *starts, int *ends, AssemblyNode *o, ResidueSet **sets, int *numSets, double rmsd)
{
	if (o->id >= 0) {
		...
		*sets = (ResidueSet *)malloc((sizeof(ResidueSet) + sizeof(int) * ma->numChains) * max);
		...
		for (i = 0; i < max; i++) {
			if (!ma->chains[index]->res[i + starts[index]].exists)
				continue;
