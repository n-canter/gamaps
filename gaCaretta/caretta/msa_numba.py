...
import psa_numba as psa
import rmsd_calculations, helper
import time
import ga_tree
...
class StructureMultiple:
    ...
    def align_from_pdb_files(input_pdb,
                             ...
                             overwrite_dssp=False,
                             ga=False, ga_lim_time=0, ga_lim_iter=0, ga_lim_same=0, ga_pop_size=0):
        """
        ...
        msa_class.align(gap_open_penalty, gap_extend_penalty,
                        ga=ga, ga_lim_time=ga_lim_time, ga_lim_iter=ga_lim_iter, ga_lim_same=ga_lim_same, ga_pop_size=ga_pop_size)
    ...
    def align(self, gap_open_penalty, gap_extend_penalty, pw_matrix=None, gamma=0.03, gap_open_sec=1., gap_extend_sec=0.1, ga=False, ga_lim_time=0, ga_lim_iter=0, ga_lim_same=0, ga_pop_size=0) -> dict:
        print("Aligning...")
        ...
        if ga:
            ...
            ctx = (self.structures, gap_open_penalty, gap_extend_penalty, pw_matrix, gamma, gap_open_sec, gap_extend_sec)
            g = ga_tree.Ga(len(self.structures), fitness, ctx, lim_time=ga_lim_time, lim_iter=ga_lim_iter, lim_same=ga_lim_same, pop_size=ga_pop_size)
            res = g.run()
            tree = np.array(res)
            alignment = align(self.structures, tree, gap_open_penalty, gap_extend_penalty, pw_matrix, gamma, gap_open_sec, gap_extend_sec)
            self.alignment = alignment
            return alignment
        
        tree = nj.neighbor_joining(pw_matrix)
    ...
    def write_superposed_pdbs(self, output_pdb_folder, alignments: dict = None):
        ...
        pdbs = superpose(alignments, self.structures)
        score(alignments, self.structures)
        output_pdb_folder = Path(output_pdb_folder)
        if not output_pdb_folder.exists():
            output_pdb_folder.mkdir()
            
        for fname, pdb in pdbs:
            pd.writePDB(str(output_pdb_folder / fname), pdb)

def align(structures, tree, gap_open_penalty, gap_extend_penalty, pw_matrix, gamma, gap_open_sec, gap_extend_sec):
    # final_structures = [s for s in structures]
    final_structures = {k: v for k, v in enumerate(structures)}
    msa_alignments = {s.name: {s.name: s.sequence} for s in structures}
    
    def make_intermediate_node(n1, n2, n_int):
        name_1, name_2 = final_structures[n1].name, final_structures[n2].name
        name_int = f"int-{n_int}"
        n1_coords = final_structures[n1].coords
        n1_coords[:, -1] *= len(msa_alignments[name_2])
        n1_coords[:, -1] /= 2.
        n2_coords = final_structures[n2].coords
        ...
        final_structures[n_int] = Structure(name_int, None, None, mean_sec, None, mean_coords)
    ...
    return {**msa_alignments[final_structures[node_1].name], **msa_alignments[final_structures[node_2].name]}

def superpose(alignments, structures):
    reference_name = structures[0].name
    reference_pdb = pd.parsePDB(str(structures[0].pdb_file))
    ...
    pdbs = [(str(f"{reference_name}.pdb"), reference_pdb)]
    for i in range(1, len(structures)):
        name = structures[i].name
        pdb = pd.parsePDB(str(structures[i].pdb_file))
        ...
        pdbs.append((str(f"{name}.pdb"), pdb))
    return pdbs

def dzero(ltarget):
    if ltarget <= 21:
        return 0.5
    return 1.24 * ((ltarget - 15) ** (1.0 / 3.0)) - 1.8

def pair_tm(a, b, ltarget):
    d0 = dzero(ltarget)
    d0 *= d0

    score = .0
    for i in range(len(a)):
        di = ((a[i][0] - b[i][0]) * (a[i][0] - b[i][0])
        + (a[i][1] - b[i][1]) * (a[i][1] - b[i][1])
        + (a[i][2] - b[i][2]) * (a[i][2] - b[i][2]))
        score += 1 / (1 + di/d0)
    return score / ltarget

def tm(core):
    tm_score = .0
    ltarget = len(core[0][1])
    for i in range(len(core) - 1):
        ltarget = min(ltarget, len(core[i][1]))
    for i in range(len(core) - 1):
        for j in range (i + 1, len(core)):
            # ltarget = min(len(core[i][1]), len(core[j][1]))
            tm_score += pair_tm(core[i][0], core[j][0], ltarget)
    n = len(core)

    return 2 * tm_score / (n * (n - 1))

def score(alignments, structures):
    reference_name = structures[0].name
    reference_pdb = pd.parsePDB(str(structures[0].pdb_file))
    core_indices = np.array([i for i in range(len(alignments[reference_name])) if '-' not in [alignments[n][i] for n in alignments]])
    aln_ref = helper.aligned_string_to_array(alignments[reference_name])
    ...
    for i in range(len(structures)):
        ...
        name = structures[i].name
        aln_name = helper.aligned_string_to_array(alignments[name])
        idxs = helper.get_alpha_indices(pdb)
        common = pdb[idxs].getCoords().astype(np.float64)[np.array([aln_name[c] for c in core_indices])]
        core.append((common, idxs))

    return tm(core)

def fitness(tree, structures, gap_open_penalty, gap_extend_penalty, pw_matrix, gamma, gap_open_sec, gap_extend_sec):
    if tree is None:
        return None
    
    try:
        start = time.time()
        alignment = align(structures, tree, gap_open_penalty, gap_extend_penalty, pw_matrix, gamma, gap_open_sec, gap_extend_sec)
        s = score(alignment, structures)
